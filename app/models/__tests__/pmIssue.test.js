...
describe('PMIssue Class', () => {
  ...
  describe('constructor', () => {
    test('should initialize with default values', () => {
      expect(pmIssue.USER_DOMAIN).toBe('CORP\\');
    });
  });
  ...
  describe('connect', () => {
    test('should connect with decoded username and password', async () => {
      ...
      await pmIssue.connect(data);

      expect(pmIssue.db.connect).toHaveBeenCalledWith('test user', 'pass', true);
    });
  ...
  describe('post', () => {
    ...
    test('should save on success', async () => {
      ...
      const result = await pmIssue.post(data);

      expect(mockDb.request).toHaveBeenCalled();
      expect(mockRequest.input).toHaveBeenCalledWith('originalUser', 'user');
      expect(mockRequest.execute).toHaveBeenCalledWith('mmspSyncInPMIssues');
    ...
    test('should throw error on save failure', async () => {
      ...
      await expect(pmIssue.post(data)).rejects.toThrow('Error occurred');
    });
  });

  describe('delete_issue', () => {
    test('should return error if no data sent', async () => {
      const result = await pmIssue.delete_issue();
      expect(result).toEqual({ error: 'No Data Sent!', num_code: 303 });
    });
    ...
    test('should save on success for delete', async () => {
      ...
      const result = await pmIssue.delete_issue(data);
  ...
  describe('check_key_fields', () => {
    test('should not throw error since no keys to check', () => {
      expect(() => pmIssue.check_key_fields({})).not.toThrow();
    });
  });

  describe('save_issue_to_db', () => {
    test('should save data to db successfully', async () => {
      ...
      const result = await pmIssue.save_issue_to_db(data);
    ...
    test('should throw error if rcode is 1', async () => {
      ...
      await expect(pmIssue.save_issue_to_db(data)).rejects.toThrow('Failure');
      expect(mockDb.request).toHaveBeenCalled();
    });
  });
  ...
  describe('array_2d_to_str', () => {
    test('should convert 2d array to string with separators and replace rsep', () => {
      const arr = [['a', 'b'], ['c\r', 'd']];
      const result = pmIssue.array_2d_to_str(arr, '\t', '\r');
      expect(result).toBe('a\tb\rc\n\td');
    });

    test('should handle single row', () => {
      const arr = [['x\n', 'y']];
      const result = pmIssue.array_2d_to_str(arr, ',', '\n');
