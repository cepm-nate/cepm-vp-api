...
class RFI {
  ...
  async post(data) {
    ...
    return await this.save_rfi_to_db(parsedData);
  }

  async delete_rfi(data) {
    if (!data || data === '') {
      return { error: 'No Data Sent!', num_code: 303 };
    }
  ...
  async save_rfi_to_db(data) {
    ...
    const dcols = data.dcols.join(',');
    const drows = this.array_2d_to_str(data.drows, fieldSep, rowSep);
    ...
    request.input('dcols', dcols);
    request.input('drows', drows);
    request.output('rcode', 0);
    request.output('ReturnMessage', '');

    const result = await request.execute('mmspSyncInRFIs');
