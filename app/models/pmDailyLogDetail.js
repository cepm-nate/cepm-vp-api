...
class PMDailyLogDetail {
  ...
  async post(data) {
    ...
    return await this.save_detail_to_db(parsedData);
  }

  async delete_detail(data) {
    if (!data || data === '') {
      return { error: 'No Data Sent!', num_code: 303 };
    }
  ...
  async save_detail_to_db(data) {
    ...
    const result = await request.execute('mmspSyncInPMDLDetails');
...
module.exports = PMDailyLogDetail;
