...
class INAdjustmentBatch {
  ...
  async post_adjustment(data) {
    ...
    return await this.save_ab_to_db(parsedData);
  }

  async delete_adjustment(data) {
    if (!data || data === '') {
      return { error: 'No Data Sent!', num_code: 303 };
    }
  ...
  async save_ab_to_db(data) {
    ...
    const result = await request.execute('mmspSyncInINAdjustmentBatch');
...
module.exports = INAdjustmentBatch;
