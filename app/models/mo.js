...
class MO {
  ...
  async post(data) {
    ...
    this.check_key_fields_mo(parsedData);

    // Connect if we need to
    await this.connect(parsedData);

    // Now we sync OUT
    return await this.save_mo_to_db(parsedData);
  }

  check_key_fields_mo(data) {
    ...
    if (!data.mo || data.mo === 'null' || data.mo === 'undefined') {
      throw new Error('"mo" is not present or undefined.');
    }
    if (!data.batchId || data.batchId === 'null' || data.batchId === 'undefined') {
      throw new Error('"batchId" is not present or undefined.');
    }
    if (!data.batchSeq || data.batchSeq === 'null' || data.batchSeq === 'undefined') {
      throw new Error('"batchSeq" is not present or undefined.');
    }
  }

  async save_mo_to_db(data) {
    const outputs = [];
    ...
    const batchSeq = data.batchSeq;
    const hdrCols = data.hdrCols.join(',');
    const hdrRow = data.hdrRow.join(fieldSep);
    const detCols = data.detCols.join(',');
    const detRows = this.array_2d_to_str(data.detRows, fieldSep, rowSep);
    ...
    request.input('batchSeq', batchSeq);
    request.input('mo', mo);
    request.input('hdrCols', hdrCols);
    request.input('hdrRow', hdrRow);
    request.input('detCols', detCols);
    request.input('detRows', detRows);
    request.output('rcode', 0);
    request.output('ReturnMessage', '');

    const result = await request.execute('mmspSyncInMOBatch');
