...
module.exports = class PhyCnts {
  ...
  // Changed (see below) 2026-01-02 - Avoid "long query" for _getChangeTrackingCurrentVersion
  // findSince(req) {
  ...
  async findSince(req) {
    ...
    try {
      ...
      return {
        PhyCounts: querySinceResult.rows,
        LAST_SEQ: currentChangeTrackingVersion,
        IsFullLoad: querySinceResult.output.ChangeTrackNum === 0, // Strict equality for clarity and safety
      ...
      throw new Error(`Failed to fetch physical counts since sequence ${req.params.last_seq}: ${error.message}`);
    }
  }
