  ...
  authenticate(username, password) {
    ...
    return new Promise(function (resolve, reject) {
      auth.authenticate(username, password, function (err, user) {
        auth.close(); // Close the connection after use to prevent accumulation
        if (err)
          reject(err);
        else if (!user)
      ...
      return this.authenticate(username, password)
        ...
        .catch(function (err) {
          ...
          if (err.name === 'InvalidCredentialsError' || (typeof err === 'string' && err.match(/no such user/i))) {
            ...
            // The error handling for recreating auth is removed since we now use local instances and close them.
            console.log(err);
